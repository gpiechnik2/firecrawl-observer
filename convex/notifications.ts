...
export const sendWebhookNotification = internalAction({
  ...
  handler: async (ctx, args) => {
    const payload = {
      ...
      change: {
        ...
        summary: args.diff?.text
          ? args.diff.text.substring(0, 200) +
            (args.diff.text.length > 200 ? "..." : "")
          : "Website content has changed",
        diff: args.diff
          ? {
              added: args.diff.text
                .split("\n")
                .filter(
                  (line) => line.startsWith("+") && !line.startsWith("+++"),
                )
                .map((line) => line.substring(1)),
              removed: args.diff.text
                .split("\n")
                .filter(
                  (line) => line.startsWith("-") && !line.startsWith("---"),
                )
      ...
      scrapeResult: {
        ...
        markdown:
          args.markdown.substring(0, 1000) +
          (args.markdown.length > 1000 ? "..." : ""),
      },
    ...
    try {
      ...
      const isLocalhost =
        args.webhookUrl.includes("localhost") ||
      ...
      if (isLocalhost) {
        ...
        if (!response.ok) {
          ...
          console.error(
            `Webhook proxy failed: ${response.status} ${errorData}`,
          );
          throw new Error(
            `Webhook proxy failed with status ${response.status}`,
          );
        }
...
export const sendEmailNotification = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (userSettings?.emailTemplate) {
      ...
      const processedTemplate = userSettings.emailTemplate
        ...
        .replace(
          /{{aiMeaningfulScore}}/g,
          args.aiAnalysis?.meaningfulChangeScore?.toString() || "N/A",
        )
        .replace(
          /{{aiIsMeaningful}}/g,
          args.aiAnalysis?.isMeaningfulChange ? "Yes" : "No",
        )
        .replace(/{{aiReasoning}}/g, args.aiAnalysis?.reasoning || "N/A")
        .replace(/{{aiModel}}/g, args.aiAnalysis?.model || "N/A")
        .replace(
          /{{aiAnalyzedAt}}/g,
          args.aiAnalysis?.analyzedAt
            ? new Date(args.aiAnalysis.analyzedAt).toLocaleString()
            : "N/A",
        );
...
export const sendCrawlWebhook = internalAction({
  ...
  handler: async (
    ctx,
    args,
  ): Promise<{ success: boolean; status: number } | undefined> => {
    ...
    try {
      ...
      if (isLocalhost) {
        ...
        if (!response.ok) {
          const errorData = await response.text();
          console.error(
            `Crawl webhook proxy failed: ${response.status} ${errorData}`,
          );
          throw new Error(
            `Webhook proxy failed with status ${response.status}`,
          );
        ...
        if (!response.ok) {
          console.error(
            `Crawl webhook failed: ${response.status} ${response.statusText}`,
          );
          throw new Error(`Webhook failed with status ${response.status}`);
        }
